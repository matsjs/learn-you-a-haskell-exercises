...
fib 0 = [0]
fib 1 = [1, 0]
fib n = fibSec !! 0 + fibSec !! 1 : fibSec
    where fibSec = fib (n-1)
...
piCalc' :: (Ord a, Fractional a, Integral b) => a -> a -> a -> b -> (a, b)
piCalc' w x y z
    | abs (x - currentPi) <= y = (currentPi, z)
    | otherwise = piCalc' (w+2) currentPi y (z + 1)
    where
        currentPi = if odd z then (x - (4/w)) else (x + (4/w))
